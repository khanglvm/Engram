...
Engram is a Rust workspace. Core code lives in `crates/`:
- `engram-daemon`: daemon lifecycle and request routing
- `engram-cli`: CLI commands (`start`, `stop`, `status`, `init`, `project`, `ping`)
- `engram-ipc`: Unix socket protocol/client/server
- `engram-indexer`: scanning, parsing, storage, file watching
- `engram-context`: context routing/rendering
- `engram-core`: shared config, metrics, project management
